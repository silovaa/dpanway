...
struct CppPathBuilder
{
   ...
   SkPath detach(){return impl->detach();}
   SkPath snapshot(){return impl->snapshot();}
   ...
   void moveTo(SkPoint p){impl->moveTo(p); }
   void lineTo(SkPoint p){impl->lineTo(p); }
   ...
   void addRect(const SkRect &r){impl->addRect(r);}
