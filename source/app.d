...
int main()
{
    try {
        ...
        WlRegistryListener.GlobalCb gcb = (ref WlRegistry, uint name, const(char)* iface, uint ver) nothrow {
                        try {
                        import std.string;  
	    ...
	    auto m_registry = WlRegistry(m_display);

        m_registry.listener = new WlRegistryListener(gcb, null);
        if (wl_display_roundtrip(m_display) < 0) 
 		    throw new Exception("wl_display_roundtrip() failed");
    }
    catch(Exception e) {
